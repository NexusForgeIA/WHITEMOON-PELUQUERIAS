...
// peluquerias-cita-mt — backend de la agenda de peluquería, MULTI-TENANT.
// v5: + cita-crear (alta manual, bloquea solape) + citas-importar (CSV en bloque, sin bloquear solape).
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ---- CITA-CREAR (panel, alta manual): opcion A, bloquea si solapa ----
    if (action === 'cita-crear') {
      ...
      const ins = await fetch(`${SUPABASE_URL}/rest/v1/citas_peluqueria`, {
        method: 'POST',
        headers: { ...REST_HEADERS, 'Prefer': 'return=representation' },
        body: JSON.stringify({ tenant: t.id, cliente_nombre: nombre, cliente_telefono: telefono, servicio, duracion_min: duracion, cita_at: citaAt, estado, notas, cliente_id: clienteId, origen: 'manual' }),
      });
      const rows = await ins.json();
      const cita = Array.isArray(rows) ? rows[0] : null;
      if (!cita) return json({ error: 'no se pudo crear la cita' }, 500);
      await log(t, cita.id, 'alta-manual', `${servicio} ${citaAt} (${nombre})`);
    ...
    // ---- CITAS-IMPORTAR (panel, CSV en bloque): carga tal cual, NO bloquea por solape ----
    if (action === 'citas-importar') {
      ...
      for (let i = 0; i < filas.length; i++) {
        ...
        try {
          ...
          const ins = await fetch(`${SUPABASE_URL}/rest/v1/citas_peluqueria`, {
            method: 'POST',
            headers: { ...REST_HEADERS, 'Prefer': 'return=representation' },
            body: JSON.stringify({ tenant: t.id, cliente_nombre: nombre, cliente_telefono: telefono, servicio, duracion_min: duracion, cita_at: citaAt, estado, notas, cliente_id: clienteId, origen: 'importado' }),
          });
          const rows = await ins.json();
      ...
      await log(t, null, 'importacion', `Importadas ${creadas}/${filas.length}${errores.length ? `, ${errores.length} errores` : ''}`);
    ...
    if (action === 'agenda') {
      ...
      const url = `${SUPABASE_URL}/rest/v1/citas_peluqueria?${enTenant(t)}&cita_at=gte.${encodeURIComponent(desde)}&cita_at=lt.${encodeURIComponent(hasta)}&select=id,created_at,cliente_nombre,cliente_telefono,cliente_id,servicio,duracion_min,cita_at,estado,resena_enviada,notas,origen&order=cita_at.asc`;
      const r = await fetch(url, { headers: REST_HEADERS });
      const rows = await r.json();
    ...
    if (action === 'estado') {
      ...
      if (estado === 'completada' && !cita.resena_enviada) {
        ...
        const pie = gmb ? `Envia al cliente el enlace de resena:\n${gmb}` : 'Configura el enlace de resenas de Google en el panel para poder enviarlo.';
